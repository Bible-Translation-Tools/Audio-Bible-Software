...
class SqlDelightVersificationDao(
    private val db: AppDatabase
) {
    private val queries = db.appDatabaseQueries

    fun fetchVersificationFile(slug: String): String? {
        return queries.fetchVersificationPath(slug).executeAsOneOrNull()
    }

    fun insert(slug: String, path: String) {
        queries.insertVersification(slug, path)
    }

    fun update(slug: String, path: String) {
        queries.updateVersification(path, slug)
    }

    fun upsert(slug: String, path: String) {
        db.transaction {
            val existing = queries.fetchVersificationPath(slug).executeAsOneOrNull()
            if (existing != null) {
                queries.updateVersification(path, slug)
            } else {
                queries.insertVersification(slug, path)
