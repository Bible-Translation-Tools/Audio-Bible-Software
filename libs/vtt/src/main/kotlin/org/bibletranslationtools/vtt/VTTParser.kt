    ...
    fun parse(inputStream: InputStream): List<WebvttCueInfo> {
        val output = object : Consumer<CuesWithTiming> {
            ...
            override fun accept(t: CuesWithTiming) {
                // add, not addLast: addLast on a List is Java 21's SequencedCollection, which
                // Android has never shipped at any API level. Identical behaviour here.
                contents.add(t)
            }
        }
        // Kotlin's readBytes(), not InputStream.readAllBytes(): the latter only reached Android
        // at API 33. Same result on every platform.
