        ...
        if (nestedElements.isEmpty()) {
            nestedElements.add(
                Element(
                    ...
                    // Kotlin's trim(), not java.lang.String.strip(): trim() is likewise
                    // Unicode-whitespace aware (unlike Java's own trim()), and resolves without
                    // depending on a Java 11+ member of String.
                    markup.trimStart('-').trim(),
                    markup.length
                )
            )
        }
