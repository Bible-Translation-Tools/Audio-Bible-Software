 ...
 *
 * Legacy projects for one book and target language are staged **together**, whatever their legacy
 * version slug, so `ulb`, `udb` and `reg` become one project with their takes merged and numbered in
 * one sequence. Each still keeps its own ledger entry and has its own legacy audio deleted, since
 * one can be staged successfully while another has an unreadable take.
 *
 * A project's units come from its source's content rows, and `deriveProjectFromVerses` — which
 * import applies for a DIALECT project — produces one unit per source verse.
 *
...
class MigrateLegacyRecorderProjects(
    ...
    /** The legacy projects that migrate as one project: one book, one target language. */
    private data class Group(
        val targetLanguageSlug: String,
        val bookSlug: String,
        val projects: List<LegacyProject>
    ) {
        val label: String get() = "$targetLanguageSlug/$bookSlug"
    ...
    override fun exec(progressEmitter: ObservableEmitter<ProgressStatus>): Completable =
        Completable.fromAction {
            ...
            val (supported, unsupported) = projects
                .filterNot { ledger.isDone(it.key) }
                .partition { it.mode == LegacyMode.VERSE }

            // Chunk mode is disabled: it needs a second source whose units are the legacy chunks,
            // and with both sources present the importer's source lookup cannot tell them apart.
            // Recorded as skipped rather than left pending, so the run can still complete.
            unsupported.forEach { project ->
                ledger.put(skipped(project, "chunk mode is not migrated by this build"))
            }

            val pending = groupsOf(supported)
            if (pending.isEmpty()) {
                installedEntityRepo.install(this)
            ...
            // One source, trimmed to the books these projects reference; see InitializeModeSources
            // for why the subset matters.
            val books = pending.map { it.bookSlug }.toSet()
            logger.info("Source required: ${modeSourceLabel()} for ${books.sorted()}")
            if (!initializeModeSources.ensureImported(mapOf(LegacyMode.VERSE to books))) {
                // Stay uninstalled and retry on the next launch rather than marking every
                // project permanently skipped.
                logger.error("Mode source unavailable; deferring migration to the next launch.")
                return@fromAction
            }
    ...
    private fun groupsOf(projects: List<LegacyProject>): List<Group> = projects
        .groupBy { it.targetLanguageSlug to it.bookSlug }
        .map { (key, grouped) -> Group(key.first, key.second, grouped.sortedBy { it.versionSlug }) }
        .sortedBy { it.label }
    ...
    private fun migrate(group: Group, ledger: MigrationLedger) {
        ...
        val sourceCollection = resolveSourceCollection(group)
            ?: return skipAll(
                group,
                ledger,
                "no ${modeSourceLabel()} source for ${group.bookSlug} — import it and relaunch"
            )
        ...
        val stageDir = File(directoryProvider.tempDirectory, "legacy-${group.targetLanguageSlug}-${group.bookSlug}")
        val staged = stageLegacyBackup.execute(
            stageDir,
    ...
    private fun resolveSourceCollection(group: Group): Collection? {
        val version = InitializeModeSources.versionFor(LegacyMode.VERSE)
        ...
        if (resolved == null) {
            ...
            val forBook = candidates
                ...
                .sorted()
            logger.error(
                "No source for ${group.label}: wanted ${modeSourceLabel()} for '${group.bookSlug}'. " +
     ...
     *
     * This is what a staged take is named after. Bridged filler rows are left out, so a range like
     * `\v 24-25` appears once as `24..25` and the takes of both verses are named after verse 24 —
     * the unit the derived project has, and so the only name import can bind them to.
     */
    ...
    /** `en/ulb/v12-verse`, for logs. */
    private fun modeSourceLabel() =
        "${InitializeModeSources.LANGUAGE}/${InitializeModeSources.IDENTIFIER}" +
                "/v${InitializeModeSources.versionFor(LegacyMode.VERSE)}"
