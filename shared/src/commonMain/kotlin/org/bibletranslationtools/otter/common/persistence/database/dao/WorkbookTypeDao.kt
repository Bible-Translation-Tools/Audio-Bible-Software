...
/**
 * The clean DAO contract for the workbook_type enum cache (no jOOQ `DSLContext` parameter). The
 * SQLDelight backend implements this directly, lazily seeding missing enum rows just like the jOOQ
 * backend does.
 */
interface WorkbookTypeDao {
    fun fetchId(mode: ProjectMode): Int
    fun fetchById(databaseId: Int): ProjectMode?
}
