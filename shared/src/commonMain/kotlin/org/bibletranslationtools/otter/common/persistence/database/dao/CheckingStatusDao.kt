...
/**
 * The clean DAO contract for the checking_status enum cache (no jOOQ `DSLContext` parameter). The
 * SQLDelight backend implements this directly, lazily seeding missing enum rows just like the jOOQ
 * backend does.
 */
interface CheckingStatusDao {
    fun fetchId(mode: CheckingStatus): Int
    fun fetchById(databaseId: Int): CheckingStatus?
}
