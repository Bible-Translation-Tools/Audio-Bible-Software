...
import org.bibletranslationtools.orature.ui.viewmodels.ChapterRestoreResult
import org.bibletranslationtools.orature.ui.viewmodels.OratureWacsLoginViewModel
import org.bibletranslationtools.orature.ui.viewmodels.OratureWacsPullUiState
...
/**
 * M5(a): Orature's port of the recorder app's `WacsPullScreen`. Entry point reached from the
 * Projects pane's "Restore from WACS" button (see
 * [org.bibletranslationtools.orature.ui.screens.OratureHomeScreen]'s `onRestoreFromWacsClick` hook,
 * wired in [org.bibletranslationtools.orature.ui.navigation.OratureNavigation] as
 * [org.bibletranslationtools.orature.ui.navigation.OratureWacsPullRoute]). Mirrors
 * [OratureWacsPublishScreen]'s three-state shape (unsupported / login / logged-in), then adds its
 * own repo -> scope+project -> restore flow:
 *
...
@Composable
fun OratureWacsPullScreen(
    loginViewModel: OratureWacsLoginViewModel,
    pullViewModel: OratureWacsPullViewModel,
    onBackClick: () -> Unit
) {
...
private fun LoggedInContent(
    username: String,
    onLogout: () -> Unit,
    state: OratureWacsPullUiState,
    viewModel: OratureWacsPullViewModel,
) {
    LaunchedEffect(Unit) {
